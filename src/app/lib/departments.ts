...
export async function getAllDepartments(domainID: string | string[] | undefined | null) {
  ...
  try {
    ...
    const res = await fetch(`http://${process.env.NEXT_PUBLIC_BASEURL}:8080/staffinformation/organizations`, {
...
export async function getAllDepartmentsHeirachy(domainID: string | string[] | undefined | null) {
  ...
  try {
    const token = await getToken("session")
    const res = await fetch(`http://${process.env.NEXT_PUBLIC_BASEURL}:8080/staffinformation/organizations/hierachy/${domainID}`, { 
      headers: { 'authorization': token },
