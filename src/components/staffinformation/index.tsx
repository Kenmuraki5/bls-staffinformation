...
const StaffProfile: React.FC<StaffInformationProps> = ({ staffData }: any) => {
  ...
  function splitJobTitles(jobTitle: string) {
    ...
    let titlesEn = parts[0].split(/, /).map(t => t.trim());
    // let titlesEn = parts[0].split(/, |\b(?!^)(?<=, )and |(?= Acting)/).map(t => t.trim());
    ...
    for (let i = 0; i < Math.max(titlesEn?.length, titlesTh?.length); i++) {
      ...
      if (engTitle && thaiTitle) {
        thaiTitle = (i != 0 && !thaiTitle?.includes("รักษาการ") && !thaiTitle?.includes("รักษาการหัวหน้าส่วน") && !thaiTitle?.includes("หัวหน้าฝ่าย") && !thaiTitle?.includes("หัวหน้าส่วนงาน")) ? "รักษาการหัวหน้าส่วน " + thaiTitle : thaiTitle;
