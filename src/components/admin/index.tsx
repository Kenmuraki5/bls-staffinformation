...
export const StartEditButtonGrid: React.FC<AdminEmployeemanagementProps & { type: "employees" | "organizations" | 'domains' | 'branchs' | "managers" | "jobs" }> = ({ data, type }) => {
  ...
  const columns_emp: GridColDef[] = [
    ...
    { field: 'logonId', headerName: 'logonId', minWidth: 120, maxWidth: 400, flex: 1, headerAlign: 'center', headerClassName: 'super-app-theme--header', align: 'center', },
  ...
  return (
    ...
    >
      <div style={{ height: '93vh', width: '100%', overflow: 'hidden' }}>
        <DataGrid
          autoHeight={false}
          rows={rows}
          ...
          onRowSelectionModelChange={(selectionModel: any) => {
            const rawIds = selectionModel?.ids ?? selectionModel; // รองรับทั้ง { ids: Set } และ Set ตรงๆ
            const stringIds = Array.from(rawIds).map(String); // แปลงเป็น string เพื่อให้ match
