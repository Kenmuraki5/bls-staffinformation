...
const Dashboard: React.FC<DashboardProps> = ({ organizations, employees }) => {
  ...
  const initialDomainId = params.domain;
  ...
  const clickHandler = useCallback(
    (event: React.MouseEvent, orgId: string, haschildren: boolean): void => {
      ...
      if (event.target instanceof Element) {
        const isIcon = !!(event.target as Element).closest(
  ...
  return (
    <main>
      <Button className='mt-5' onClick={toggleTreeViewVisibility}>{isTreeViewVisible ? "Hide" : "Show"}</Button>
      <div className='flex flex-row w-screen px-5'>
        <div className='h-full border-2 rounded p-2' style={{ width: isTreeViewVisible ? '350px' : '0' }}>
          {isTreeViewVisible && (
            <SimpleTreeView
              defaultExpandedItems={allIds}
              aria-label="customized"
              sx={{ overflowX: 'hidden', minHeight: 270, flexGrow: 1, maxWidth: 500 }}
            >
              {sortedData.map((node) => renderTree(node))}
            </SimpleTreeView>
          )}
        </div>
        <div className='w-screen mx-3 p-3 border-2 border rounded bg-white' style={{ width: isTreeViewVisible ? 'calc(100% - 370px)' : '100%' }}>
